...
SEX <- "M"
#Now getting significance bars to cut & paste
res_CSF_F <- run_apoe_model(
  DABNI_CSF[DABNI_CSF$Sex == SEX,],
  apoe_col = "APOE_grouped",
  y_col = "CSFAB42_AB40",
  age_col = "Age_at_CSF",
  cohort = "cohort",
...
p1 <- res_CSF_F[[1]] + xlab("Age") + ylab("CSF AB42/AB40") + scale_shape_manual(values = c(1, 4, 4))
...
lemon::grid_arrange_shared_legend(res_CSF_by_sex$plot + ggtitle("A."),
                                  res_CL_by_sex$plot + ggtitle("B."),
                                  res_pTau217_DABNI_by_sex$plot + ggtitle("C."),
                                  res_pTau217_ABCDS_by_sex$plot + ggtitle("D."),
